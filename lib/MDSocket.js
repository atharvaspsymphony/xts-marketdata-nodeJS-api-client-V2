...
module.exports = class MDEmitter {
  ...
  init(reqObject) {
    ...
    this.socketMD.socketMarketData.on('1501-json-full', (data) => {
      this.eventEmitter.emit(settings.socket.Touchline, data);
    });
    ...
    /**
     * Listener of the 1501-json-full event via socket and emit the touchlineEvent via event Emitter
     *
     *
     * @event 1501-json-full
     */
    this.socketMD.socketMarketData.on('1501-json-full', async (data) => {
      let marketDataObject = await this.constructFullTouchLineObject(
        settings.enums.marketDataPorts.touchlineEvent,
        data
      );
      this.eventEmitter.emit(settings.socket.touchlineEvent, marketDataObject);
    });
  ...
  /*touchLineDataEvent listener for event emitter*/
  ontouchLineEvent(fn) {
    this.eventEmitter.on(settings.socket.touchlineEvent, (data) => {
      fn(data);
    });
  }
  ...
  async constructFullTouchLineObject(messageCode, data){
    var marketData = JSON.parse(data);
    if (marketData.Touchline.LastTradedTime)
      marketData.Touchline.LastTradedTime = this.getBarTimeFromTouchLineInfo(
        parseInt(marketData.Touchline.LastTradedTime),
    ...
    marketData =  {
      ...
      "ExchangeSegment":marketData.ExchangeSegment,
      "ExchangeInstrumentID":marketData.ExchangeInstrumentID,
      "ExchangeTimeStamp":marketData.ExchangeTimeStamp,
      "LastTradedPrice":marketData.Touchline.LastTradedPrice,
      "LastTradedQunatity":marketData.Touchline.LastTradedQunatity,
      "TotalBuyQuantity":marketData.Touchline.TotalBuyQuantity,
      "TotalSellQuantity":marketData.Touchline.TotalSellQuantity,
      "TotalTradedQuantity":marketData.Touchline.TotalTradedQuantity,
      "AverageTradedPrice":marketData.Touchline.AverageTradedPrice,
      "LastTradedTime":marketData.Touchline.LastTradedTime,
      "LastUpdateTime":marketData.Touchline.LastUpdateTime,
      "PercentChange":marketData.Touchline.PercentChange,
      "Open":marketData.Touchline.Open,
      "High":marketData.Touchline.High,
      "Low":marketData.Touchline.Low,
      "Close":marketData.Touchline.Close,
      "TotalValueTraded":marketData.Touchline.TotalValueTraded,
      "BidInfo":marketData.Touchline.BidInfo,
      "AskInfo":marketData.Touchline.AskInfo,
    }
